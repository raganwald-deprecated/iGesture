...
jQuery.fn.gesture = function () {
	
	var global_settings = {
		
	};

	// when a function is given, this is a function that accepts a gesture directly,
	// as in $(...).gesture(function (gesture) {...})
	return function(fn, settings) {
		...
		// them.
			...
			function (argv, gesture_events) {
				...
				jQuery.each(gesture_names, function (gesture_name) {
					$(this).bind('gesture_' + gesture_name, function (event) {
							if (event.unhandled) {
								event.unhandled = false;
